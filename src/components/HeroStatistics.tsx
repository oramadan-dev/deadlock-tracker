                ...
                {columns.map((metric) => <th
                  ...
                  <button
                    ...
                    <span aria-hidden="true">
                      {preferences.sort === metric.id ? preferences.direction === 'desc' ? ' ↓' : ' ↑' : ' ↕'}
                    </span>
                  </button>
                </th>)}
              </tr>
                  ...
                  return <td key={metric.id}>
                    {(metric.id === 'winRate' || metric.id === 'pickRate') && value !== null ? (
                      <span
                        ...
                        role="meter"
                        aria-label={row.hero.name + ' ' + metric.label}
                        aria-valuemin={0}
                        aria-valuemax={100}
                        aria-valuenow={Math.min(100, Math.max(0, value * 100))}
                        ...
                        <span className="percentage-label percentage-label-remainder" aria-hidden="true">
                          {(100 - Math.min(100, Math.max(0, value * 100))).toFixed(1)}
                          %</span>
                      </span>
                    ) : formatted}
                  </td>
        ...
        <fieldset className="radio-filter">
          ...
          {([7, 30] as const).map((days) => <label key={days}>
            <input
              ...
              checked={preferences.date.kind === 'rolling' && preferences.date.days === days}
              onChange={() => update({ date: { kind: 'rolling', days } })} />
            {days} days</label>)}
          <label>
            <input
              type="radio"
              name="statistics-date"
              checked={preferences.date.kind === 'all'}
              onChange={() => update({ date: { kind: 'all' } })} />
            All</label>
          ...
          {seasonStart !== null && <label>
            <input
              type="radio"
              name="statistics-date"
              checked={preferences.date.kind === 'season'}
              onChange={() => update({ date: { kind: 'season', start: seasonStart } })} />
        ...
        <fieldset className="radio-filter">
          ...
          <label>
            <input
              ...
              checked={preferences.matchMode === 'ranked'}
              onChange={() => update({ matchMode: 'ranked' })} />
            Ranked only</label>
          <label title="Ranked and unranked">
            <input
              type="radio"
              name="statistics-matches"
              checked={preferences.matchMode === 'ranked,unranked'}
              onChange={() => update({ matchMode: 'ranked,unranked' })} />
            All</label>
        </fieldset>
      </div>
