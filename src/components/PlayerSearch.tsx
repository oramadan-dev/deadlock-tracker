...
export function PlayerSearch() {
  ...
  return <>
    <form className="player-search" onSubmit={(event) => { event.preventDefault(); void search() }}>
      ...
      <input id="player-name" name="player" type="search" placeholder="Steam name or account ID" autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} />
      <button type="submit">Search</button>
    </form>
    {state.status === 'loading' && <p role="status">Loading player…</p>}
    {state.status === 'error' && <div role="status">{state.message} <button className="theme-toggle" onClick={() => retryAction.current()}>Retry</button></div>}
    {state.status === 'candidates' && <section className="player-candidates" aria-label="Choose a player">
      ...
      {!state.profiles.length ? <p>No indexed profiles found. Try an account ID.</p> : <>
        <p className="statistics-note">Indexed Steam profiles · Select an account to continue{state.profiles.length === 20 ? ' · Showing up to 20 results; refine your name if needed' : ''}</p>
        <ul>{state.profiles.map((profile) => <li key={profile.account_id}>
          <button className="candidate-select" onClick={() => { void select(profile.account_id) }}>
            <img src={profile.avatarmedium} alt="" width="44" height="44" onError={(event) => { event.currentTarget.style.visibility = 'hidden' }} />
            <span><strong>{profile.personaname || 'Account ' + profile.account_id}</strong><small>Account {profile.account_id} · {profile.matches_played_last_30d} matches in 30 days</small></span>
          </button><a href={steamProfileUrl(profile.account_id)} target="_blank" rel="noreferrer">Steam profile</a>
        </li>)}</ul>
      </>}
    </section>}
    {selected ? <PlayerDashboard key={selected.revision} accountId={selected.accountId} initialStats={selected.stats} initialWindow={selected.window} select={(id) => { void select(id) }} overview={overview} /> : <HeroStatistics />}
  </>
}
